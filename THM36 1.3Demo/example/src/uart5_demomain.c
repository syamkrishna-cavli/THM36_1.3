...
#define UART5_TEST_DATALENGHT     16
...
/* Private variables ---------------------------------------------------------*/
__align(4) uint8_t UART5_TxBuf[2048];
__align(4) uint8_t UART5_RxBuf[2048];

static __align(4) LLI_StructTypeDef UART5_receive_lli[5];
...
void TMC_UART5_MspInit(UART5_HandleTypeDef *huart)
{
    ...
    GPIO_PinAFConfig(GPIOB,GPIO_PIN_Source2,GPIO_AF4_UART5);/*TMC(20240409)-gsx:DMA+IDLE soft flow control GPIOB2 need set to output mode,At this point, the line needs to be annotated*/
...
void UART5_DemoMain(void)
{
	...
	if(Mode == Uart5_Demo_DMA)
	{
        ...
        while(1)
        {
            if (receive_flag)/*TMC(20240409)-gsx:DMA has receive configure length*/
            {
                receive_flag = 0;
